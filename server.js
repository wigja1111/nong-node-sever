...
async function initSchema() {
  ...
  try {
    ...
    // 🔹 Kakao 로그인용 user_kakao_id 컬럼 (이미 있으면 에러 무시)
    try {
      await conn.query(
        'ALTER TABLE `users` ADD COLUMN `user_kakao_id` BIGINT UNSIGNED NULL UNIQUE AFTER `user_email`'
      );
    } catch (e) {
    ...
    try {
      ...
      if (byKakao.length) {
        ...
        if (!userRow) {
          ...
          const [r] = await conn.execute(
            `INSERT INTO users (user_name,user_email,user_password,user_role,user_kakao_id)
             VALUES (?,?,?,?,?)`,
